...
for($x = 5; 1 <= $x; $x--){
              ...
              //  echo $general->make_date($from,'time').'====to===='.$general->make_date($to,'time');
                $total_comment = $db->selectAll($general->table(13),"where sender_id!=".PAGE_ID." and created_time BETWEEN ".$from." AND ".$to,'count(comment_id) as total');
                $total_comment = intval($total_comment[0]['total']);
                $total_reply = $db->selectAll($general->table(13),"where sender_id=".PAGE_ID." and replyed=1 and replyTime BETWEEN ".$from." AND ".$to,'replyTime,assignTime');

                 foreach($wrapup as $wrap){
                $reply = $db->selectAll($general->table(13),"where sender_id=".PAGE_ID." and wuID=".$wrap['wuID']." and replyed =1 and replyTime BETWEEN ".$from." AND ".$to);
                $wrapusData[$range_of_dates[$x]][$wrap['wuID']]=count($reply);
                }
                foreach($scentiment as $sc){
                $reply = $db->selectAll($general->table(13),"where sender_id=".PAGE_ID." and scentiment=".$sc." and replyed =1 and replyTime BETWEEN ".$from." AND ".$to);
