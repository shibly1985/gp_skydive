...
<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        ...
        <div class="x_title">
            <h2>Sentiment <small>Wall Post</small></h2>
...
<?php
    ...
    $query="SELECT wuID,scentiment, COUNT(scentiment) as total FROM ".$general->table(12)." WHERE scentiment in (".implode(',',array_keys($scentiments)).") AND wuID in (".implode(',',array_keys($wrapup)).") AND created_time between ".$from." AND ".$to." group by wuID,scentiment";
    $total_sen=$db->fetchQuery($query);

    foreach($total_sen as $t){
        if(isset($data[$t['wuID']][$t['scentiment']])){
            $data[$t['wuID']][$t['scentiment']]+=$t['total'];    
        }
        else{
            $data[$t['wuID']][$t['scentiment']]=$t['total'];
        }
                ...
                <table class="table table-striped users_view_table">
                    ...
                    <tbody>
                        <?php
                            ...
                            $jArray2['xData'] = array('Wrap-up','Positive','Nutral','Negetive');
