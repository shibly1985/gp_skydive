<?php
    ...
    if($c!=false&&!isset($_GET['flush'])){
        ...
        //        $jArray[__LINE__]=__LINE__;
        if($uID!=0){
            $logins=$db->selectAll($general->table(18),'where ulsStartTime between '.$from.' and '.$to.' and uID='.$uID);
            $firstLogin=0;
            $lastActivity=0;
            $active=0;
            if(!empty($logins)){
                foreach($logins as $l){
                    ...
                    $active+=$general->timestampDiffInArray($l['ulsStartTime'],$l['ulsLastActivity'],true);
                }
            } 
            $fl=0;
            $la=0;
            if($firstLogin>0){
                $fl=$firstLogin;
                $firstLogin=date('d-M-Y h:i:s A',$firstLogin);
            }
            else{
                $firstLogin='-';
            }
            if($lastActivity>0){
                ...
                if(date('d-m-Y',$firstLogin)!=date('d-m-Y',$firstLogin)){
                    $lastActivity=date('d-M-Y h:i:s A',$lastActivity);
                }
                else{
                    $lastActivity=date('d-M-Y h:i:s A',$lastActivity);
                }
            ...
            $tLogin=$general->timestampDiffInArray($la,$fl,true);
            ...
            //if($uID==106)$echo='d';
            $totalReply=0;
            $reply = $db->selectAll($general->table(13),"where sender_id!=".PAGE_ID." and replyTime BETWEEN ".$from." AND ".$to.' and isDone!=1 and replyBy='.$uID,'count(comment_id) as tr',$echo);
            $totalReply+=intval($reply[0]['tr']);
            $reply = $db->selectAll($general->table(14),"where sender_id!=".PAGE_ID." and replyTime BETWEEN ".$from." AND ".$to.' and isDone!=1 and replyBy='.$uID,'count(comment_id) as tr',$echo);
            $totalReply+=intval($reply[0]['tr']);
            $reply = $db->selectAll($general->table(12),"where replyTime BETWEEN ".$from." AND ".$to.' and isDone!=1 and replyBy='.$uID,'count(post_id) as tr',$echo);
            $totalReply+=intval($reply[0]['tr']);
            ...
            $tDone+=intval($reply[0]['tr']);
            $reply = $db->selectAll($general->table(12),"where replyTime BETWEEN ".$from." AND ".$to.' and isDone=1 and replyBy='.$uID,'count(post_id) as tr');
            $tDone+=intval($reply[0]['tr']);
            ...
            $rData=array(
                ...
                'tHours'    => $general->makeTimeAvgI($active+$tb),
                'tLogin'    => $general->makeTimeAvgI($tLogin),
                'reply'     => $totalReply,
                'tHide'     => $tHide,
                'done'      => $tDone,
                'transfer'  => $tTransfer,
